...
class ScoreManager {
	...
	getCurrentLevel() {
		...
		const totalAttempts = Object.values(this.scores).reduce(
			(sum, score) => sum + score.attempts,
			0,
		);
		const totalCorrect = Object.values(this.scores).reduce(
	...
	getNextLevel() {
		...
		return currentIndex < this.levels.length - 1
			? this.levels[currentIndex + 1]
			: null;
	}
	...
	populateLevelInfo() {
		...
		if (nextLevel) {
			...
			const pointsFromPrevious =
				currentLevelIndex > 0 ? this.levels[currentLevelIndex].minPoints : 0;
			const pointsToNext = nextLevel.minPoints - pointsFromPrevious;
			const currentProgress = totalCorrect - pointsFromPrevious;
			const progressPercentage = Math.min(
				100,
				Math.max(0, (currentProgress / pointsToNext) * 100),
			);
	...
	populateIndividualScores() {
		...
		programList.innerHTML = scoreEntries
			.map(([key, score]) => {
				const addressTypes = ["IPv4", "IPv6", "MAC", "none"];
				const typeBreakdown = addressTypes
					.map((type) => {
						// Safety check - ensure byType exists and has the type
						const typeData = score.byType?.[type]
							? score.byType[type]
	...
	resetAllScores() {
		if (
			confirm(
				"Are you sure you want to reset all scores and streak? This cannot be undone.",
			)
		) {
			this.scores = {};
			this.resetStreak();
