...
const shortcutVariants: Partial<Record<QuizButtonVariant, string>> = {
  answer: "absolute top-3 left-3 w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold shadow-sm bg-indigo-500 text-white",
  "answer-selected": "absolute top-3 left-3 w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold shadow-sm bg-white text-blue-600",
  "answer-correct": "absolute top-3 left-3 w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold shadow-sm bg-white text-green-600",
  "answer-incorrect": "absolute top-3 left-3 w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold shadow-sm bg-white text-red-600",
};

export const QuizButton = forwardRef<HTMLButtonElement, QuizButtonProps>(
  ({ variant = "primary", size = "md", children, shortcut, disabled, className, onClick, ...props }, ref) => {
    const baseClasses = buttonVariants[variant];
    const sizeClasses = sizeVariants[size];
    const showShortcut = shortcut && (variant.startsWith("answer"));
    const shortcutClasses = showShortcut ? (shortcutVariants[variant] || shortcutVariants.answer || "") : "";
