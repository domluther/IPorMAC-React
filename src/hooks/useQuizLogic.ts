...
export function useQuizLogic({ 
  ...
  const handleAnswerSelect = useCallback((answerId: number, isCorrect: boolean, questionData?: any) => {
    // Record score with configurable points
    const questionKey = `quiz-${Date.now()}`;
    const earnedPoints = isCorrect ? correctPoints : 0;
    scoreManager.recordScore(questionKey, earnedPoints, maxPoints, questionData?.type, questionData?.address);
